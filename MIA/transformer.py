...
class TransformerClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, n_heads, dropout, max_len=256):
        super().__init__()
        ...
        encoder_layers = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True  # Add this line for batch-first processing
        )
    ...
    def forward(self, text):
        ...
        positions = torch.arange(0, text.size(1)).unsqueeze(0).repeat(text.size(0), 1).to(device)

        # Word embeddings + position encodings
        embedded = self.dropout(self.embedding(text) + self.pos_embedding(positions))

        # Create attention mask (ignore padding tokens)
        src_mask = (text == 0).to(device)

        # Transformer encoding
        encoded = self.transformer_encoder(embedded, src_key_padding_mask=src_mask)

        # Take first token's output as classification features
        features = encoded[:, 0, :]  # Modified this line since batch dimension is first
...
def run_experiment(run_id):
    ...
    n_layers = 2
    n_heads = 4
    dropout = 0.1  # Typically use smaller dropout for Transformers
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.0001)  # Typically use smaller learning rate for Transformers
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), f'best_transformer_model_run_{run_id}.pt')
    ...
    # Load best model
    model.load_state_dict(torch.load(f'best_transformer_model_run_{run_id}.pt'))
...
def main():
    ...
    for run_id in range(1, 6):
        ...
        results_df.to_csv(f'results_transformer/run_{run_id}_results.csv', index=False)
    ...
    # Save results
    combined_results.to_csv('results_transformer/all_runs_combined.csv', index=False)
    final_results.to_csv('results_transformer/median_results.csv', index=False)
