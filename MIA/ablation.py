...
for item in data_set:
    key = (item["model"], item["task_id"],item["benchmark"])
    grouped[key]["indices"].append(item["index"])
    grouped[key]["perplexity"].append(item["perplexity"])
    grouped[key]["lev"].append(item["lev"])
    grouped[key]["jac"].append(item["jac"])
    grouped[key]["cos"].append(item["cos"])
    grouped[key]["vec_dis"].append(item["vec_dis"])
    grouped[key]["manhattan"].append(item["manhattan"])
    grouped[key]["code"].append(item["code"])
    ...
    grouped[key]["benchmark"] = item["benchmark"]
...
result = [
    {
        ...
        "jac": group["jac"],
        "cos": group["cos"],
        "vec_dis": group["vec_dis"],
        "manhattan": group["manhattan"],
...
experts_features=[]
# feature_name = ["pass","codebleu","perplexity","lev"]
...
class ImprovedModel(nn.Module):
    def __init__(self, input_dim):
        ...
        # Third convolutional layer: further compression
    ...
    def forward(self, x):
        ...
        # Convolutional part
        x = F.relu(self.bn_conv1(self.conv1(x)))  # [batch, 1, input_dim/2]
        x = F.relu(self.bn_conv2(self.conv2(x)))  # [batch, 1, input_dim/4]
...
model = ImprovedModel(input_dim=5 * 256).to(device)
...
if len(feature_name)>0:
    ...
    merged_features = np.hstack([answer_features,merged_features])
else:
    merged_features = np.hstack([answer_features, conv_features_np])
...
# Save as CSV
output_file = f"experts_all/median_experts_{len(feature_name)}.csv"  # Modify filename to reflect content
