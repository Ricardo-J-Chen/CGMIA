...
class TextCNNClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim, output_dim, dropout, max_len=256):
        super().__init__()
        ...
        self.fc = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(128 * len(self.convs), 256),  
            nn.BatchNorm1d(256),
            nn.ReLU(),
    ...
    def forward(self, text):
        ...
        pooled = [conv(embedded).squeeze(2) for conv in self.convs]
...
def main():
    ...
    for run_id in range(1, 6):
        ...
        results_df.to_csv(f'results_cnn/run_{run_id}_results.csv', index=False)
    ...
    combined_results.to_csv('results_cnn/all_runs_combined.csv', index=False)
    final_results.to_csv('results_cnn/median_results.csv', index=False)
